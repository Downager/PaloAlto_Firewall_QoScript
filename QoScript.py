...
# 傳入 report.csv 三日內超流名單，並提取 nowlock.csv [鎖定日期, 超流使用者]
def create_nowlock(dfUserList):

    # 取得歷史資料內連續超流三日名單
    usercount = dfUserList.groupby('使用者名稱').size()
    # 修改 usercount == 3 調整連續超流 N 天便鎖定
    ...
    f.close()
    print('#202 create_nowlock(): 已更新 nowlock.csv \n 連續超流三日名單: \n', dfNewlock)


# 讀取 nowlock.csv，刪除已鎖定三天以上帳號，並回傳刪除過後之 dataframe
def delete_lockedNdays(dfLocked):
    # 刪除已鎖定三天的帳號
    # 修改 days=3 調整超流使用者鎖定 M 天
...
# 設定各項參數
paloaltoip = '***.***.***.***' # FIXME: Replace This
apikey = (getKey('username', 'password', paloaltoip))  # FIXME: Replace This
jobid = (getJobid('custom', 'Top 50 Wireless User traffic daily report', apikey, paloaltoip))
reportstatus = (getReportstatus(jobid, apikey, paloaltoip))
print('#225: ', 'apikey: ', apikey, 'jobid, ', jobid)
print('#226: ', reportstatus)
...
while reportstatus[0] != 'success':
    jobid = (getJobid('custom', 'Top 50 Wireless User traffic daily report',
                      apikey, paloaltoip))
    errorCount += 1
    print('#235 response.status = error, 重新產生job-id', '\n', 'new-jobid: ',
          jobid, '\n Count: ', errorCount)
...
while reportstatus[1] != 'FIN':
    ...
    actCount += 1
    print(
        '#241 job.status = ACT, 每15秒更新狀態直到 job.status = FIN. \n new-jobstatus: ',
    ...
    if actCount == 30:
        jobid = (getJobid('custom', 'Top 50 Wireless User traffic daily report', apikey, paloaltoip))
        actCount = 0
        print('#249 超過重試次數 30 (15秒*30次), 重新產生job-id', '\n new-jobid: ', jobid)

# 取得報告回傳值 (day_of_receive_time, srcuser, bytes_total , bytes_received, bytes_sent)
day_of_receive_time, srcuser, bytes_total, bytes_received, bytes_sent = (getReport(jobid, apikey, paloaltoip))
...
mask = dfOverFlow['總流量'] >= 50000000000
...
print('#290 delete_Ndaysago_report(): 已移除超過三日之歷史超流名單: report.csv')
...
while not (os.path.exists('nowlock.csv')) and (os.path.getsize('nowlock.csv')):
    time.sleep(3)
    print('#298: nowlock.csv 不存在，三秒後重試')
...
print('#305: delete_lockedNdays()，已移除鎖定三天以上帳號: nowlock.csv')

overflowuser = dfLockedremoved['超流使用者'].values.tolist()
print('#308: 超流名單', overflowuser)


# 確認超流 list是否為空，若為空則只推送使用者'11111'進去
if overflowuser:
    # 推送超流名單進 PA 以及取得 response.status 回傳值
    pushResult = pushOverflowuser(overflowuser, apikey, paloaltoip)

    while pushResult != 'success':
        pushResult = pushOverflowuser(overflowuser, apikey, paloaltoip)
        print('#318 推送失敗，間隔一秒後重新推送 \n new-pushResult: ', pushResult)
        time.sleep(1)
else:
    emptylist = ['11111']
    pushResult = pushOverflowuser(emptylist, apikey, paloaltoip)

    while pushResult != 'success':
        pushResult = pushOverflowuser(overflowuser, apikey, paloaltoip)
        print('#326 推送失敗，間隔一秒後重新推送 \n new-pushResult: ', pushResult)
        time.sleep(1)

print('#329: pushCommit() 結果: ', pushCommit('', apikey, paloaltoip))
